...
- **Langfuse** is configured in `langfuse_client.py`, on its own `TracerProvider` — *not* the global one Logfire uses. Both SDKs are OpenTelemetry-native and by default attach to the global `TracerProvider`; sharing it turned out not to work cleanly, since Langfuse's default export filter only forwards spans with `gen_ai.*` attributes or from known LLM instrumentation scopes, which would silently drop the manual Logfire spans this file creates. An isolated provider needs its own explicit spans, so `ask()` wraps its body in a Langfuse `span` (input=query, output=result, a `grounded` score) and `generation._call_local`/`_call_api` each wrap their body in a Langfuse `generation` (model, input=prompt, output=answer, token usage — enables native cost tracking for the `api` backend).
- **Langfuse sessions & user feedback** (`app.py`): each `ask()` call is wrapped in `propagate_attributes(session_id=...)` using Gradio's `request.session_hash` — a fresh random id per browser tab load, so refreshing the page starts a new Langfuse session by design; the current session is shown in the UI so the demo audience can see it. `run_query` also accepts an explicit `session_id` override for non-Gradio callers (see `scripts/seed_demo_metrics.py`, which mints one `uuid4`-based session per script run so repeated runs don't collapse into one). Two 👍/👎 buttons score the most recent trace via `langfuse.create_score(..., name="user_feedback")`, using a `trace_id` `ask()` now returns on its result dict specifically so a caller outside the span can still attach a score after the fact.
- **Langfuse Prompt Management** (`generation.py`, `scripts/setup_langfuse_prompt.py`): `PROMPT_TEMPLATE` moved out of hardcoded Python into a Langfuse-managed prompt named `rag-answer`, fetched via `langfuse.get_prompt(PROMPT_NAME, fallback=FALLBACK_PROMPT_TEMPLATE, cache_ttl_seconds=300)` and rendered with `.compile(...)` (Langfuse's `{{var}}` syntax, not Python `.format()`). The old hardcoded string is kept as the `fallback=` value rather than deleted — per Langfuse's caching docs, the SDK caches prompts client-side after the first fetch, so this fallback is only ever exercised on a cold cache with Langfuse unreachable; the app's behavior with Langfuse down is otherwise unaffected. The resolved prompt client is threaded into `_call_local`/`_call_api`'s `prompt=` kwarg, which links each generation observation to the exact prompt version that produced it — visible via `promptName`/`promptVersion` on the observation, one of the clearer "Langfuse does this, Logfire has no equivalent" differentiators. `scripts/setup_langfuse_prompt.py` is the one-time upload of the initial version under the `production` label (`get_prompt` defaults to that label).
...
- **Not yet implemented, planned as follow-up work** (each its own step, not bundled): a **Langfuse dashboard** mirroring the existing Logfire one, **Datasets + Experiments** tied to `evaluate.py`'s existing batch comparison flow, and a Langfuse **Monitor/Alert** mirroring the Logfire alert.
