...
- **Langfuse sessions & user feedback** (`app.py`): each `ask()` call is wrapped in `propagate_attributes(session_id=...)` using Gradio's `request.session_hash` — a fresh random id per browser tab load, so refreshing the page starts a new Langfuse session by design; the current session is shown in the UI so the demo audience can see it. Two 👍/👎 buttons score the most recent trace via `langfuse.create_score(..., name="user_feedback")`, using a `trace_id` `ask()` now returns on its result dict specifically so a caller outside the span can still attach a score after the fact.
...
- **Not yet implemented, planned as follow-up work** (each its own step, not bundled): Langfuse **Prompt Management** (move `generation.PROMPT_TEMPLATE` into a versioned, UI-editable Langfuse prompt), a **Langfuse dashboard** mirroring the existing Logfire one, **Datasets + Experiments** tied to `evaluate.py`'s existing batch comparison flow, and a Langfuse **Monitor/Alert** mirroring the Logfire alert.
