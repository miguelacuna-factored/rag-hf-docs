-- Row 3 by-backend comparison:
-- Name: Truncated vs. complete — Claude
-- Description: Within the Claude Haiku backend, the split between truncated and complete generations.
-- Truncated means the answer hit the max_tokens cap and got cut off before reaching a
-- natural stopping point, instead of ending because the model was actually done.
-- Type: Pie Chart
SELECT
  CASE WHEN (attributes->>'truncated')::boolean THEN 'Truncated' ELSE 'Complete' END AS label,
  count(*) AS value
FROM records
WHERE span_name = 'api generation complete'
  AND attributes->>'backend' = 'claude-haiku-4-5-20251001'
GROUP BY label
